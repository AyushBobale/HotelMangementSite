...
def getUserDetails(username):
  sql1 = "SELECT * FROM USERID WHERE USERNAME = ? "
  con = create_connection(DATABASE)
  cursor = con.cursor()
  cursor.execute(sql1, (username,))
  row = cursor.fetchall()
...
def getInvoiceBill(username):
  userid = getuserid(username)
  sql1 = "SELECT * FROM HOMEDELIVERY WHERE USERID = ? AND PLACED = ? AND DELIVERED = ? "
  con = create_connection(DATABASE)
  cursor = con.cursor()
  cursor.execute(sql1, (userid,'True','None'))
  row = cursor.fetchall()
  return row
